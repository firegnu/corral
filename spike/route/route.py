#!/usr/bin/env python3
"""路由试验：把一段任务摘要交给 TypeSafe 的 Jev，一次请求问完「几档、交给哪家、要不要交叉审查」。

只给建议，不做决定。只用标准库。key 从环境变量 TYPESAFE_API_KEY 读。
用法：echo "<任务摘要>" | route.py [--agent 名字="负责的范围" ...]
输出一行 JSON，每一项带 verdict（None 表示拿不准，用主控的判断）；调不通时 {"ok": false, "error": ...}，退出码 1。
"""
import argparse
import json
import os
