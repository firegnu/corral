...
- 面板每个 agent 一行：名字、种类、实例编号、状态、正在做（在干活或卡住时显示最近的工具和这一轮跑了多久）、多久没输出、接入窗口数、最近一次输入的来源、终端标题。状态列按状态上色（`working` 黄、`blocked` 红、`idle` 绿）；行首红色 `!` 表示卡住，绿色 `▶` 表示右格正在显示它；接入列标紫表示它在别处接入着。下半部分是选中 agent 的上一轮回复，PgUp / PgDn 翻页。
- 面板里的键：↑↓（或 j / k）选；回车或鼠标点一行，让右格显示它；`x` 再按 `y` 关掉它；`q` 退出。
...
- 面板只调用 `corral ls`、`status`、`reply`；只有你按回车（接入）和 `x` 再按 `y`（关掉）时才动 agent。
