...
- 面板每个 agent 一行：名字、种类、实例编号、状态、正在做（在干活或卡住时显示最近的工具和这一轮跑了多久）、多久没输出、接入窗口数、最近一次输入的来源、终端标题。卡住（`blocked`）的行标红，右格正在显示的行前面有 `▶`。下半部分是选中 agent 的上一轮回复，PgUp / PgDn 翻页。
...
- 那个 agent 已经在别的窗口接入时，右格只能看不能打字，面板会提示。
