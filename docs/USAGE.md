...
corral start demo/alice --cwd ~/proj -- omp --approval-mode yolo
...
在终端里分出左右两格（任何能分屏的终端都行，两个并排的窗口也行），各运行一次：

```sh
<仓库>/tools/board                 # 左格：面板，每 3 秒刷新，q 退出
<仓库>/tools/board --viewer        # 右格：显示器，面板选了谁它就接入谁
<仓库>/tools/board --prefix demo/  # 面板只看某个前缀
<仓库>/tools/board --bell          # agent 做完一轮或卡住时终端响一声
<仓库>/tools/board --once          # 只打印一次状态表
```

- 面板按名字里第一个 `/` 前面的部分分组，组内名字省掉前缀。每个 agent 一行：名字、种类、实例编号、状态、正在做（在干活或卡住时显示最近的工具和这一轮跑了多久，在干活的前面转圈）、多久没输出、接入窗口数、最近一次输入的来源、工作目录、终端标题。左格窄到一行放不下时不藏列：第一行照旧是名字、状态、正在做、多久没输出，种类、实例、接入、来源、目录、标题在下面按项折行（短的合并、长的另起一行），前面一根灰色引线 `│` 把这几行拴在名字上、最后一行用 `╰` 收口；再窄时「正在做」挪到名字下面单独一行。这时表头行也省掉。
- 颜色和行首标记：状态列 `working` 黄、`blocked` 红、`idle` 绿。行首红色 `!` 是卡住；黄色 `?` 是疑似卡住（启动超过 60 秒，或在干活却 120 秒没有任何输出，多半是被 Esc 打断了；秒数用 `--stuck-start`、`--stuck-quiet` 改）；青色 `●` 是做完了一轮、你还没看过，选中它或在右格显示它就消失；绿色 `▶` 是右格正在显示它。在干活的「无输出」超过 30 秒变黄、到疑似卡住变红。接入列标紫表示它在别处接入着。有 agent 退出时底栏提示。
- 选中 agent 的上一轮回复默认不显示（和右格重复），按 `r` 在框的下半部分显示，再按一次隐藏；常见的 Markdown（标题、列表、代码块、行内代码、粗体）简单渲染；放不下时最右一列有滚动条，鼠标停在回复上用滚轮滚动，也可以用 PgUp / PgDn（Mac 上是 Fn+↑ / Fn+↓）翻页。隐藏时不去取回复。
- agent 多到一屏放不下时，列表最右一列有滚动条，鼠标停在列表上用滚轮滚动（不改变选中的是哪个）；上面、下面藏住的 agent 数写在框边右侧，如 `↑ 4`、`↓ 3 · ! 1 · ● 1`，藏住的里面有卡住、疑似卡住、做完没看的分别计数。
- 面板里的键：↑↓（或 j / k）选；回车或鼠标点一行，让右格显示它；`r` 显示或隐藏上一轮回复；`s` 在按名字排和按状态排（卡住的在前）之间切换；`x` 再按 `y` 关掉它；`q` 退出。
- 显示器就是替你在右格里运行 `corral attach`：换人时先断开当前的，被断开的 agent 继续跑。在右格按 Ctrl-] 断开，或者 agent 退出，右格回到空闲画面；空闲时按 `q` 退出。点完 agent 要再点一下右格才能打字。
- 那个 agent 已经在别的窗口接入时，面板不让右格接（后接入的窗口只读，画面尺寸跟着先接入的窗口走，会乱），提示先在那边按 Ctrl-] 断开。所以 corral 启动的 agent 最好都从面板看。
- 两边先开哪个都行，互不依赖；显示器同时只能开一个。它们通过 `$TMPDIR/corral-board-<用户号>/` 里的小文件传话，和 corral 的状态目录无关。
- 面板只调用 `corral ls`（目录也来自这里）、`status`、`reply`（显示回复时才调）；只有你按回车（接入）和 `x` 再按 `y`（关掉）时才动 agent。
...
- **pi 或 omp 刚 start 就「不存在」**：多半是你的某个全局扩展出错，让 pi 一启动就退出。直接在终端里敲 `pi` 或 `omp` 看报错。
...
- **agent 升级之后**：先跑一遍 `<仓库>/tools/smoke`，四家真实 agent 各跑一组检查，大约 8 分钟、花少量 token。有不通过的，先弄清是上游变了还是 corral 的问题，再接着用。
