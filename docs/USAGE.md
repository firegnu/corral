...
- 面板按名字里第一个 `/` 前面的部分分组，组内名字省掉前缀。每个 agent 一行：名字、种类、实例编号、状态、正在做（在干活或卡住时显示最近的工具和这一轮跑了多久，在干活的前面转圈）、多久没输出、接入窗口数、最近一次输入的来源、工作目录、终端标题。左格窄时按「标题、实例、目录、最近输入、种类、无输出」的先后藏列。
...
- 下半部分是选中 agent 的上一轮回复，常见的 Markdown（标题、列表、代码块、行内代码、粗体）简单渲染；放不下时最右一列有滚动条，鼠标停在回复上用滚轮滚动，也可以用 PgUp / PgDn（Mac 上是 Fn+↑ / Fn+↓）翻页。
- 面板里的键：↑↓（或 j / k）选；回车或鼠标点一行，让右格显示它；`s` 在按名字排和按状态排（卡住的在前）之间切换；`x` 再按 `y` 关掉它；`q` 退出。
...
- 面板只调用 `corral ls`（目录也来自这里）、`status`、`reply`；只有你按回车（接入）和 `x` 再按 `y`（关掉）时才动 agent。
