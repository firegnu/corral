...
第三轮（2026-09-25）：影响面回测。编排技能要按影响面（看得见 / 改行为 / 碰要害）定验证预算，交叉审查的四道题已经在判「碰要害」，这一轮试加一道是非题判「看得见」（只影响显示、文档、只读研究）。脚本 `spike/route/impact_eval.py`，三种措辞放进同一个请求一起问；案例是两个真实项目做完的 29 件活加 1 条合成的「只换一个颜色」，摘要是主控当时送给路由的原文，人标三档（看得见 8、改行为 13、碰要害 9），案例数据不进仓库。

| 措辞 | 三档全对（原摘要，门槛 0.8） | 说明 |
|---|---|---|
| A「只改外观或文字，或只调研写报告，不改程序做什么」 | 24/30 | 文档、只读研究 0.91–0.97；排版类显示活只有 0.40–0.78 |
| B「做错了一眼看得见、容易改回，不影响数据、逻辑、输入」 | 23/30 | 分不开：修 bug、加滚动这类也给 0.5–0.84，有一条误报 |
| C「只限于外观、文字、文档或只读研究，没有新功能、不改输入处理和逻辑」 | 25/30 | 改行为的最高只到 0.39；排版类显示活 0.49–0.86 |

错的几条分两类。一类不怪新题：交叉审查那四题拿不准，碰要害的活落到了改行为（2 条），「调研安装脚本风险」按安全题判成碰要害（1 条，标注本身可争议）。另一类是排版类显示活没过门槛（m16、m21、m28）：它们的原摘要里写了测试覆盖、真实终端录制等验证计划，读起来像大活。

按新技能的写法（摘要只写做什么、动哪、做错了影响什么，不写验证计划）改写 8 条再问：看得见的三条 A 0.89–0.93、C 0.91–0.95；改行为的五条 A 最高 0.17、C 最高 0.22；B 仍分不开（改行为的给到 0.51–0.79）。改写是知道标注的人写的，结果可能偏乐观。
