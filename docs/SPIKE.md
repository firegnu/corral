...
## 路由试验（2026-09-19，编排技能用，不属于内核）

用分类模型给主控的分派建议「几档、交给哪家、要不要交叉审查」。代码在 `spike/route/`（`route.py` 一次请求问完，`eval.py` 对照人标答案）；案例是一个真实项目里做完的 13 件任务（后端 6、前端 4、杂活 3），摘要中英文各一份，案例数据不进仓库。模型 `jev-1.13.0`，每次约 660 个输入 token。

| 判断 | 中文摘要 | 英文摘要 | 说明 |
|---|---|---|---|
| 交给哪家（两家加「都不是」） | 12/13 | 12/13 | 界线清楚的全对，置信度 1.00；唯一判错的是「写脚本核对数据」，置信度 0.26 / 0.51 |
| 几档（轻、常规、重） | 9/13 | 8/13 | 分歧都在边界上（骨架算轻还是常规、规则多的后端算常规还是重）；判错的置信度大多低于 0.8，有一条 0.80 |
| 要不要交叉审查（四道是非题，门槛 0.5） | 9/13 | 9/13 | 判错的全是多报：「配置目录权限、只监听本机」报了安全，「导入数据、审定规则」报了数据模型和核心规则，有几条说得通 |

结论：中文摘要和英文一样好用，不用翻译。「交给哪家」可以交给它；「几档」和「交叉审查」要改题目（它按字面读，比如把「写入默认数据」当成改数据模型），并由人裁定有分歧的标注，再跑一轮。

第二轮（同日）：加了第三家「杂活：项目代码之外的清理、一次性脚本、核对数据、跑命令汇报」，并在代码里规定杂活只有在「有把握的轻档」时才交给它（它用的模型弱）；按门槛直接给结论（几档、交给哪家置信度 ≥ 0.8 才给；交叉审查任何一题 ≥ 0.8 要、四题都 ≤ 0.2 不要，其余交回主控）；数据模型那道题写明「只读写数据不算」。中文摘要的结果：

| 判断 | 一致 | 拿不准（交回主控） | 会被采用的错判 |
|---|---|---|---|
| 几档 | 7 | 6 | 0 |
| 交给哪家 | 12 | 1（杂活，但轻档置信度 0.78，被规则挡回） | 0 |
| 交叉审查 | 8 | 3 | 2（都是多报：「只监听本机、目录权限」报安全 0.88；「导入数据、审定规则」报数据模型和核心规则 0.88） |

三件杂活都选对了第三家（置信度 1.00、0.92、1.00）。改数据模型那道题的措辞只把误报从 0.92 降到 0.88，没解决。英文摘要结果相近（几档 6 / 7 / 0，交叉审查 7 / 4 / 2）。
