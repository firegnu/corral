...
- Which agent: the controller follows one split table in the skill (Claude Code for design and taste: architecture, data model and UI proposals, plus UI work itself; Codex for implementation whose approach is already set, work either could do, and cross-review; pi for light chores outside the product code).
