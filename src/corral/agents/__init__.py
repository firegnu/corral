"""agent 适配：认识 Claude Code、Codex 和 pi，按启动参数给它们加钩子、带首句。

钩子只随 corral 启动的那个 agent 生效，不写任何全局配置。Claude Code、Codex 的钩子命令用 /usr/bin/python3 运行
agent 状态目录里的 hook.py 副本；pi 用 --extension 加载状态目录里的 hook_pi.ts 副本。都不引用 corral 仓库和
...
ADAPTERS = {a.kind: a for a in (Claude(), Codex(), Pi())}
