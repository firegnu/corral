"""agent 适配：认识 Claude Code 和 Codex，按启动参数给它们加钩子、带首句。

钩子只随 corral 启动的那个 agent 生效，不写任何全局配置。钩子命令运行 agent 状态目录里的 hook.py 副本，
用 /usr/bin/python3，不引用 corral 仓库和 start 时的 Python（DESIGN 5.3）。
"""
import os
...
ADAPTERS = {a.kind: a for a in (Claude(), Codex())}
